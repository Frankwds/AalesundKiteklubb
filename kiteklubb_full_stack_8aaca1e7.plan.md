...
- Protects `/courses/*/chat` routes (requires authentication + enrollment)

### 3d. Auth Helpers

- `src/lib/auth/index.ts` -- `getCurrentUser()` helper that calls `supabase.auth.getUser()` and fetches the user's role from `public.users` via Supabase SDK. Used for UI-level decisions (showing admin nav, etc.), but NOT for security -- RLS handles that.
